...
### Added
- **New brands: Hiseeu (3) + Srihome (1)** (#268) — budget Wi-Fi cameras from the manufacturer/retailer listings: Hiseeu `TZ-HBY08` (8 MP 4K wireless bullet), `C90B` (4 MP solar/battery), `WHD205` (5 MP PTZ); Srihome/Sricam `SH057` (4 MP dual-band Wi-Fi, starlight, H.265). Frigate RTSP configs from the #268 RTSP-patterns layer (Hiseeu `/live/main`@80, SriHome `/1/h264major`); the solar/battery C90B is an app/cloud camera (no local RTSP).
