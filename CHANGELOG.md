...
Dataset grows to **12,403 cameras / 159 brands** (+5 Hikvision models added while re-sourcing). A data-enrichment + provenance pass across four lanes: **System-on-Chip (SoC)** from community firmware projects, **IK impact rating** extracted from cameras' own feature text, specific **image-sensor part numbers**, and a large **Hikvision OEM re-sourcing** effort (#164).
...
- **Re-sourced 161 Hikvision records** from reseller mirrors (`download.axilogi.com`, `manuals.plus`, `scribd`, hikvision-peru, …) to their **official `assets.hikvision.com` / `hikvision.com` datasheet PDFs** — each URL verified (curl 200 where the CDN allows; the `content/dam` PDFs sit behind an Akamai JS challenge and were confirmed from the official product listing). **Hikvision reseller-only 192 → 31**; dataset-wide #165 flag 726 → 568.
- **Added 5 new Hikvision models** surfaced during re-sourcing, all from official datasheets: **DS-2CD2386G2H-I** (8MP DarkFighter turret, no-audio `-I`), **DS-2CD2086G2H-I2U/SL** (8MP white-strobe + audible-deterrence bullet), and the **DS-2CV2×xxG2-IDW Wi-Fi family** — DS-2CV2021G2-IDW (2MP bullet), DS-2CV2141G2-IDW (4MP dome), DS-2CV2121G2-IDW (2MP dome, H.264).
- **~31 records remain** on reseller mirrors (exact-SKU official docID not yet located) — tracked for a follow-up pass. The `DS-2CD2043G2-L` record is a probable truncated duplicate of `DS-2CD2043G2-LI` (left for review, not re-sourced).
