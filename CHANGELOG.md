...
Dataset reaches **12,290 cameras / 158 brands** (+283). Full **Vivotek** and **Amcrest** catalogue sweeps, a dataset-wide **`network.ethernet_speed_mbps`** backfill from official datasheets, missing **Uniview** and **GeoVision** bullet/dome models, and a round of sourcing/config fixes.
...
### Added — Homaxi (new brand, +11)
- New brand **Homaxi** (Chinese ONVIF-OEM) from official `homaxi.com` datasheets: ColorView 24/7 full-color turret/bullet/dome (`IPC8TF4R4`/`R8`, `IPC8BF4R8`, `IPC8DV4R4-I2-TPMCZ`), Active-Deterrence AD2 bullet/dome with red/blue strobe + two-way audio (`IPC8BV6R8-AD2`, `IPC8BF6R8-AD2`, `IPC8DV4R8-AD2`), a 32x auto-tracking PTZ (`IPT651R5-Z32-AD-TPMSCR`), an 8MP dual-lens 180° panoramic (`IPC8MST2R8-AD-TPMSC`), a 6MP fisheye (`IPC8FE1R6-I2-TPMS`), and an ANPR/LPR bullet (`IPC8BV6R4-LPR`). ONVIF/RTSP; Homaxi publishes no fixed RTSP path so configs use ONVIF auto-config; `ethernet_speed_mbps: 100`; `ndaa_compliant: true` where the datasheet states NDAA (ColorView/PTZ/fisheye lines), omitted on the AD2 active-deterrence models.
