# Digital Watchdog DWC-MPV72Wi28ATW

| Field | Spec |
|-------|------|
| Brand | Digital Watchdog |
| Model | DWC-MPV72Wi28ATW |
| Type | dome |
| Connectivity | ethernet |
| Resolution | 1080p (2.1MP, 1920×1080) |
| Sensor | 1/2.8" CMOS |
| Lens | 1× 2.8mm |
| Field of view | 111H x 62V° |
| Night vision | ir (24m), 0.05 lux, 0.05 lux color |
| Power | PoE / 12V DC |
| Storage | microSD ≤ 1024GB, NVR |
| Protocols | onvif, rtsp, http |
| IP rating | IP67 |
| IK rating | IK10 |
| Two-way audio | No |
| Operating temp | -20 to +50°C |
| Released | 2025 |

## Features

- Star-Light Plus color-in-darkness
- True WDR 120dB
- Smart IR
- 3D DNR
- IVA+: intrusion detection, line crossing, loitering, counting, object detection, tailgating, direction, stopped vehicle, appear/disappear, object left, logical rules
- 16 privacy masks
- Motion detection
- Tamper detection
- De-fog
- Day/night auto switching
- NDAA compliant
- UL Listed

## Sources

- https://digital-watchdog.com/productdetail/DWC-M(P)V72WixATW/
- https://digitalwatchdog.s3.us-west-1.amazonaws.com/_downloads_2025/DW_DS_DWC-MV72WixATW_Rev0225.pdf

---
*Auto-generated from digital-watchdog-dwc-mpv72wi28atw.json — do not edit by hand.*
