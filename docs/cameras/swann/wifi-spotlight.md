# Swann WiFi Spotlight Camera

*Also known as: Swann 2K WiFi Spotlight Outdoor Camera*

| Field | Spec |
|-------|------|
| Brand | Swann |
| Model | WiFi Spotlight Camera |
| Type | bullet |
| Connectivity | wifi |
| Resolution | 2K QHD (4MP, 2560×1440) |
| Lens | 1× Fixedmm F2.0 |
| Field of view | 120 diagonal° |
| Night vision | color (12m) |
| Power | Hardwired (USB power adapter) |
| Storage | microSD ≤ 128GB |
| Protocols | http |
| IP rating | IP65 |
| Two-way audio | Yes |
| Released | 2024 |

## Features

- 2K WiFi spotlight outdoor camera
- color night vision with spotlight
- True Detect PIR heat sensing
- built-in siren
- SAFE by Swann App
- Alexa / Google

## Sources

- https://support.swann.com/hc/en-us/articles/4714813018905-SWIFI-SPOTCAM-Outdoor-Wireless-Security-Camera

---
*Auto-generated from swann-wifi-spotlight.json — do not edit by hand.*
